...
| Model discovery | App-server documents `model/list`, including visible/hidden model entries, reasoning options, input modalities, and the default model marker. [App-server model/list](https://developers.openai.com/codex/app-server) | A direct subscription HTTP model catalog is not documented. API model pages and app-server catalogs are not interchangeable, and account/workspace eligibility can differ. Live checks found GPT-6 IDs accepted by the generation endpoint but omitted from `/codex/models`. | Keep `/codex/models` as live account discovery without synthesizing rows. Allow explicitly selected IDs absent from that response to reach generation; the service remains authoritative for access and effort validation. |
| ellmer seam | The official ellmer source exposes `chat_openai()` as a Responses API factory with `base_url`, dynamic `credentials`, `api_headers`, and `service_tier`; its OpenAI stream parser consumes output-text deltas and terminal response events. [ellmer OpenAI provider](https://github.com/tidyverse/ellmer/blob/main/R/provider-openai.R) | The observed subscription terminal event can omit output that was present in deltas. ellmer 0.4.2 also does not assemble fragmented Responses function-call argument events. A native provider would require unstable/unexported upstream seams. | Pin exactly ellmer 0.4.2 and keep one version-gated provider/Chat execution seam. Structured output, registered function tools, async modes, rich content, and clone-safe lifecycle behavior are covered by that seam; tool invocation remains on ellmer's helpers. |
| OAuth cache | httr2 provides encrypted OAuth token caching, refresh handling, cache pruning, and a user-level cache path. [httr2 OAuth documentation](https://httr2.r-lib.org/articles/oauth.html) | Cached refresh tokens remain usable by R code that can use httr2; cache location and retention should be visible to users. | The package uses an OAuth client named `ellmercodex` with httr2's encrypted cache. Tests use a temporary cache and fixture tokens; live persistence/refresh is manual acceptance evidence, never an automatic test. |
...
- No test, helper, example, package load hook, or CI test step may open a
  browser, start OAuth, read credentials, write to a user cache, or make an
...
- Tests must assert protocol shape, redaction, status classification, framing,
  ...
  timing.
- OAuth cache integration, refresh rotation, and browser callback tests belong
  in a manual acceptance script or an explicitly opt-in local test profile.
  They must use a package-owned httr2 client name and a temporary cache.
...
3. ellmer exposes a stable, exported integration/finalization seam or the
   ...
   covered by the exact ellmer 0.4.2 gate and compatibility tests.
4. httr2 cache behavior, refresh-token rotation, logout scope, and failure
   recovery are tested without touching real user credentials in CI.
